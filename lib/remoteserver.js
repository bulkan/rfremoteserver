...
RemoteServer.prototype.pass = function(callback) {
  return callback(null, {status: 'PASS'});
};
...
RemoteServer.prototype.start_remote_server = function(conf) {
  ...
  for (name in self) {
    if (["start_remote_server", "library", "create_callback"].indexOf(name) == -1) {
