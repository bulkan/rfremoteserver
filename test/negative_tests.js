...
var options = {host: 'localhost', port: 4242, timeout: 100};
...
describe('Broken & Bad Keywords', function(){
  ...
  it('long running keywords should timeout', function(done){
    client.methodCall('run_keyword', ['dont call me baby'], function(err, value){
      if (err) return done(err)
      value.should.have.property('error');
      value.should.have.property('traceback');
      value.status.should.be.equal('FAIL');
